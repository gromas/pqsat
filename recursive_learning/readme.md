...
Без удаления переменных из P, размер BDD на уровне i определяется как Size(BDDi)=exp(|V|-∑|Qj|), при j=i..n. При этом результирующий BDD представляет собой компактно сжатые знания (ALL-SAT), время получения (ВСЕХ возможных решений) оценивается как O(|T|*poly(Q)), где T - все возможные решения для данной функции.
